...
figure(); hold on; grid on;
plot(t_arr'./60, w_body(1,:)-w_body_g(1,:), 'DisplayName', '\omega_x');
plot(t_arr'./60, w_body(2,:)-w_body_g(2,:), 'DisplayName', '\omega_y');
plot(t_arr'./60, w_body(3,:)-w_body_g(3,:), 'DisplayName', '\omega_z');
xlabel('t (min)'); ylabel('\omega (rad/s)');
legend();
% xlim([0 12]);
hold off
...
figure(); hold on; grid on;
plot(t_arr'./60, quat_out(1,:)-quat_out_g(1,:), 'DisplayName', 'q_1');
plot(t_arr'./60, quat_out(2,:)-quat_out_g(2,:), 'DisplayName', 'q_2');
plot(t_arr'./60, quat_out(3,:)-quat_out_g(3,:), 'DisplayName', 'q_3');
plot(t_arr'./60, quat_out(4,:)-quat_out_g(4,:), 'DisplayName', 'q_4');
xlabel('t (min)'); ylabel('q'); 
legend(); 
% xlim([0 12]);
hold off
...
% Propagate angular velocity and attitude
[w_body_g, quat_out_g, rv_ECI_out, M_out] = propagate_attitude_grav(t_arr, w0, q0);
